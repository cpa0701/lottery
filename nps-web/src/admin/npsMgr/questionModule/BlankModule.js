...
export default class BlankModule extends PureComponent {
    render() {
        const {index, questionName, questionType, isView, questionNameBlur, onChange, isBlank, jumpDescribe = null, connDescribe = null, infoView} = this.props;
        return (
            <div className={'radio-module'} style={{padding: '12px'}}>
                ...
                <TextArea className="textAreaInput" questionindex={index} isblank={isBlank} onBlur={onChange}
                    // defaultValue={optionList}
                          autosize={questionType === '03' ? false : {minRows: 3, maxRows: 3}}/>
                {infoView ? <div style={{
                    display: 'inline-block',
                    marginLeft: '8px',
                    marginTop: '5px',
                    color: '#efa030'
                }}>{jumpDescribe ? "* " + jumpDescribe : ''} {connDescribe ? "*" + connDescribe : ''}</div> : ''}
            </div>
        );
    }
}